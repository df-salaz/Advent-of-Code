...
fn main() {
    ...
    let mut total = 0;
    let mut anode_matrix: Vec<Vec<char>> =
        vec![vec!['.'; input_matrix[0].len()]; input_matrix.len()];
...
fn place_anode(anode_matrix: &mut [Vec<char>], anode_y: i32, anode_x: i32, total: &mut i32) {
...
fn oob(y: i32, x: i32, matrix: &[Vec<char>]) -> bool {
